...
int main(void)
{
        ...
        /* Ê¹ÓÃ¼òÒ×³õÊ¼»¯³õÊ¼»¯PTE¶Ë¿ÚµÄ6Òý½Å×÷ÎªIOÊ¹ÓÃ ÉèÖÃÎªÍÆÍìÊä³öÄ£Ê½ */
    GPIO_QuickInit(HW_GPIOC, 6, kGPIO_Mode_OPP);
    /* ÁíÍâÒ»ÖÖ³õÊ¼»¯·½Ê½: ½á¹¹ÌåÄ£Ê½µÄ³õÊ¼»¯ ÓÐµãÀàËÆSTM32¹Ì¼þ¿â*/
...
//    GPIO_Init(&GPIO_InitStruct1);
    ...
    /* ¿ØÖÆPTE¶Ë¿ÚµÄ6Òý½ÅÊä³öµÍµçÆ½ */
    GPIO_WriteBit(HW_GPIOC, 6, 1);    /* Ê¹ÓÃ¼òÒ×³õÊ¼»¯³õÊ¼»¯PTE¶Ë¿ÚµÄ6Òý½Å×÷ÎªIOÊ¹ÓÃ ÉèÖÃÎªÍÆÍìÊä³öÄ£Ê½ */
   // GPIO_QuickInit(HW_GPIOC, 6, kGPIO_Mode_OPP);
    /* ÁíÍâÒ»ÖÖ³õÊ¼»¯·½Ê½: ½á¹¹ÌåÄ£Ê½µÄ³õÊ¼»¯ ÓÐµãÀàËÆSTM32¹Ì¼þ¿â*/
//    GPIO_InitTypeDef GPIO_InitStruct1;
//    GPIO_InitStruct1.instance = HW_GPIOE;
...
//    GPIO_Init(&GPIO_InitStruct1);
    ...
    /* ÔÚPC01 ÉÏ²úÉúÒ»¶¨ÆµÂÊµÄ·½²¨ ±ãÓÚÂö³å¼ÆÊý²âÁ¿ */
    FTM_PWM_QuickInit(FTM1_CH1_PA13, kPWM_EdgeAligned, 1000);
    /* ÉèÖÃFTM µÄÕ¼¿Õ±È */
    FTM_PWM_ChangeDuty(HW_FTM1, HW_FTM_CH1, 5000); // 50%Õ¼¿Õ±È 0-10000 ¶ÔÓ¦ 0-100%
    
    /*²âËÙÇý¶¯*/
    //LPTMR_PC_QuickInit(LPTMR_ALT2_PC05); /* Âö³å¼ÆÊý */
    ...
    /* ¿ªÆôÒ»¸öPIT¶¨Ê±Æ÷ ²úÉú1SÖÐ¶Ï ÔÚÖÐ¶ÏÖÐ¶ÁÈ¡LPTMRµÄ¼ÆÊýÖµ */
    ...
    FTM_PWM_ChangeDuty(HW_FTM1, HW_FTM_CH0,PWM_Motor); // 0 - 10000  to 0% - 100% ÓÒÂÖ
    FTM_PWM_ChangeDuty(HW_FTM1, HW_FTM_CH1,PWM_Motor); // 0 - 10000  to 0% - 100%   ×óÂÖ
    //²ÎÊýµ÷Õû
